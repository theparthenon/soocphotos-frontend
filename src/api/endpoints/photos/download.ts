...
import type { BaseQuery, BaseQueryResult, DownloadResponse, StatusResponse } from "@/@types/photos/download";
...
async function startDownloadProcess(baseQuery: BaseQuery<DownloadResponse>, image_hashes: string[]) {
    return (
        await baseQuery({
            method: "POST",
            body: { image_hashes },
            url: EndpointUrls.tasksPhotosDownload
        })
    ).data!;
}
...
async function checkDownloadStatus(baseQuery: BaseQuery<StatusResponse>, job_id: string) {
    return (
        await baseQuery({
            url: EndpointUrls.tasksPhotosDownload + '?job_id=' + job_id
        })
    ).data!;
}

async function downloadFile(baseQuery: BaseQuery<Blob>, filename: string) {
    const response = await baseQuery({ url: EndpointUrls.tasksDownload + filename, responseHandler: r => r.blob() });
...
export const downloadApi = api.injectEndpoints({
    endpoints: builder => ({
        [Endpoints.downloadPhotos]: builder.query<void, { image_hashes: string[] }>({
            ...
            queryFn: async ({ image_hashes }, baseQueryApi, _extraOptions, baseQuery ) => {
                ...
                const { job_id: jobId, url: filename } = await startDownloadProcess(
                    baseQuery as BaseQuery<DownloadResponse>, image_hashes
                );
                ...
                const statusInterval = setInterval(async () => {
                    ...
                    switch (status) {
                        case "SUCCESS":
                            ...
                            await baseQuery({ method: "DELETE", url: EndpointUrls.tasksZipDelete + filename});
