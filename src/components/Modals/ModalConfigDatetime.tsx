...
import { getRuleExtraInfo } from "@/components/Settings/DateTimeSettings";
...
export function ModalConfigDatetime({ opened, onClose, availableRules, onAddRules }: Props) {
  ...
  const rules = availableRules
    ...
    .map(rule => (
      <tr key={rule.name} className="dateTimeRule">
        <td>
          <strong>
            {rule.name} (ID:{rule.id})
          </strong>
          <div className="dateTimeRuleType">Rule: {rule.rule_type}</div>
