...
import { useLazyFetchAutoAlbumQuery } from "@/api/endpoints/albums/auto";
...
export default function AlbumAutoGalleryView() {
  ...
  function formatTimestamp(timestamp: string): string {
    return DateTime.fromISO(timestamp).setLocale(LOCALE).toLocaleString(DateTime.DATE_MED);
  }
  ...
  const subtitle = ` ${album.photos.length} Photos, ${timestampFrom} - ${timestampTo}`;
              ...
              <div key={v[0]}>
                ...
                <Group>
                  ...
                  <div>
                    <Title order={5}>
                      Day { i + 1 } -{" "}
                      {DateTime.fromISO(v[0]).setLocale(LOCALE).toLocaleString(DateTime.DATE_HUGE)}
                    </Title>
                    <Text color="dimmed">
